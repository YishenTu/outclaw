...
| `oc coding list [--json] [--repo <repo-id-or-path>] [--all] [--by repo\|recent]` | List registered repositories and recent coding sessions |
| `oc coding cancel <provider/session>` | Interrupt the active coding turn without rolling back files |
| `oc coding status <provider/session> [--block] [--timeout N] [--json]` | Print running, error, cancelled, or the final response |
...
- Run these commands from your default cwd, even when operating on a project repo. Supply the repo as an explicit path or registered id (`oc coding start /absolute/path/to/repo "..."`).
...
- Use `list --json` when you need a structured index of registered repositories and recent coding sessions. The default list is flat and sorted by recent activity; use `--by repo` for a grouped human view.
- Use `cancel` to interrupt an active coding turn. It exits after the daemon accepts the cancellation request. The session remains resumable and existing filesystem edits are not rolled back.
- Use `status` when you need a non-streaming snapshot. It prints `running` while the latest turn is active, `error: <message>` for failed turns, `cancelled` for interrupted turns, or `done` plus the final assistant response when the session is idle.
- Use `status --json` for machine-readable status. Fields such as `last_prompt`, `final_response`, and `error` are state-dependent and may be absent.
- Use `status --block` to wait silently until the session reaches `done`, `error`, or `cancelled`. Pair it with `--timeout N` when a caller needs a bounded wait; timeout exits 124.
...
- `status`, `list`, `cancel`, and `transcript` exit 0 on successful lookup. `status --block` exits 124 on timeout. Usage, daemon, or session lookup failures exit 1.
