...
import { createBrowserSidebarWatcher } from "../../../../src/runtime/browser/sidebar/watcher.ts";
...
describe("createBrowserSidebarWatcher", () => {
	...
	test("maps inbox changes onto inbox invalidations", () => {
		vi.useFakeTimers();
		const watchers = new Map<string, FakeWatcher>();
		const events: Array<{
			agentId?: string;
		...
		watcher.start();
		watchers.get("/workspace")?.emit("agents/alpha/inbox/todo.md");
		vi.advanceTimersByTime(100);
	...
	test("ignores .git index churn caused by status reads", () => {
		...
		watcher.start();
		watchers.get("/workspace")?.emit(".git/index");
		watchers.get("/workspace")?.emit(".git/index.lock");
		vi.advanceTimersByTime(100);
	...
	test("keeps meaningful .git metadata changes observable", () => {
		vi.useFakeTimers();
		const watchers = new Map<string, FakeWatcher>();
		const events: Array<{
			agentId?: string;
		...
		watcher.start();
		watchers.get("/workspace")?.emit(".git/HEAD");
		vi.advanceTimersByTime(100);
